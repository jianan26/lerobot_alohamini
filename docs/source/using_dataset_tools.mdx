...
6. **Replace Actions** - Replace each action with the next frame's state within the same episode
...
#### Replace Actions with Next States

Replace the action at time `t` with `observation.state` at time `t+1`. The last frame of each episode uses its own state. The `action` and `observation.state` features must have the same shape.
