...
On the Pi, start the existing AlohaMini Host first, then start the dedicated client. The client creates and owns a local SSH tunnel to the server and uses the Pi user's existing SSH key.
