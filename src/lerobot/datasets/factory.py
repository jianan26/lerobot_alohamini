...
def resolve_delta_timestamps(
    ...
    """
    delta_timestamps = {}
    relative_state_delta_indices = getattr(cfg, "relative_state_delta_indices", None)
    for key in ds_meta.features:
        if key == REWARD and cfg.reward_delta_indices is not None:
            delta_timestamps[key] = [i / ds_meta.fps for i in cfg.reward_delta_indices]
        if key == ACTION and cfg.action_delta_indices is not None:
            delta_timestamps[key] = [i / ds_meta.fps for i in cfg.action_delta_indices]
        if key == OBS_STATE and relative_state_delta_indices is not None:
            delta_timestamps[key] = [i / ds_meta.fps for i in relative_state_delta_indices]
